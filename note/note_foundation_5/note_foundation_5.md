...
Java 中的接口是对单继承的一个补充，在开发过程中像增强类的功能，可以借助接口实现，接口是多实现（比如一个人可以有多个老师）
...
3. **静态方法和默认方法**：从JDK 8开始，接口可以包含静态方法和默认方法（用 `default`关键字定义），静态方法可以通过接口名直接调用，而默认方法在实现类中可以被重写。
4. **私有方法**：从JDK 9开始，接口支持私有方法，可以用来辅助实现接口中的默认方法或抽象方法。
